...
const URLS = {
    ...
    REDEEM_DANONE: "/api/APIs/Promotions/api/Danone/RedeemVoucher",
...
async function processMerida(phone) {
    try {
        ...
        // الاستجابة من Merida قد لا تكون JSON دائماً
        let data;
        try { data = await response.json(); }
        catch(e) { 
            console.warn("Merida response was not JSON");
            data = { data: { redeemOutputs: { RedeemErrorDoc: { errDesc: "Success" } } } }; // افتراض النجاح إذا لم تكن JSON
        }
...
// Forgot Password (لا توجد مشكلة هنا أصلاً)
...
// Ramadan 500 ميجا (لا توجد مشكلة هنا)
...
// =================== دوال مساعدة (كما هي - صحيحة) =====================

// دالة تسجيل الدخول الأساسية (SignInUser)
...
// دالة تسجيل الدخول الكاملة (Generate Basic Auth Token)
